...
const SLACK_CHANNEL_ID_PARAMETER_STORE_NAME =
  process.env.SLACK_CHANNEL_ID_PARAMETER_STORE_NAME;
const SLACK_TOKEN_PARAMETER_STORE_NAME =
  process.env.SLACK_TOKEN_PARAMETER_STORE_NAME;
...
export const handler = async () => {
  try {
    const slackToken = await getParameterStoreValue(
      SLACK_TOKEN_PARAMETER_STORE_NAME,
    );
    const slackChannelId = await getParameterStoreValue(
      SLACK_CHANNEL_ID_PARAMETER_STORE_NAME,
    );
    ...
    const oneHourAgoUnixTimestamp = dayjs()
      .tz("Asia/Tokyo")
      .subtract(1, "hour")
      .unix()
      .toString();
    ...
    if (result.messages && Array.isArray(result.messages)) {
      const messages = result.messages.map((msg) => ({
        ...
        formattedDate: dayjs
          .unix(msg.ts ? Number.parseFloat(msg.ts) : 0)
          .tz("Asia/Tokyo")

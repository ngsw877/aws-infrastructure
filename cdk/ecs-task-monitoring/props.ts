import { type StackProps, aws_logs as logs } from "aws-cdk-lib";
...
export const getEcsTaskMonitoringStackProps = (
  accountName: string,
): EcsTaskMonitoringStackProps => {
  ...
  if (!stackProps) {
    ...
    console.error(
      `利用可能なアカウント: ${Object.keys(accountStackPropsMap).join(", ")}`,
    );
    process.exit(1);
  }

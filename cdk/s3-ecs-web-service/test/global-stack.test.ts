...
describe('GlobalStack', () => {
  ...
  const tenantsWithIpAddresses = params.globalStackProps.tenants.filter(
    tenant => tenant.allowedIpAddresses && tenant.allowedIpAddresses.length > 0
  );
  ...
  test('ACM証明書にプライマリドメインと代替ドメインが正しく設定されている', () => {
    ...
    const alternativeDomains = params.globalStackProps.tenants.slice(1).map(tenant => tenant.appDomainName);
  ...
  test('IP制限の除外パスが正しく適用されている', () => {
    ...
    if (!params.globalStackProps.ipRestrictionExcludedPaths || params.globalStackProps.ipRestrictionExcludedPaths.length === 0) {
